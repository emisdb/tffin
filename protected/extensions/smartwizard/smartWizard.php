...
class smartWizard extends CWidget {
  public  $selected=0,  // Selected Step, 0 = first step   
    ...
    $contentPane='<div id="step-{stepNumber}"><h2 class="StepTitle">Step {stepNumber}: {StepTitle}</h2>{content}</div>';
			...
			<!-- Tabs -->
			  		<div id="<?php echo strtr($this->selector,array("#"=>''));?>" class="swMain row-fluid">
			  			<ul>
			  			<?php echo $tabs;?>
			  			</ul>
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            //the css to use
            $this->styles=$assets. '/styles/smart_wizard.css';
				//Yii::app() ->clientScript -> registerCssFile($baseUrl . '/styles/smart_wizard.css', CClientScript::POS_HEAD);	
			    Yii::app() ->clientScript -> registerScriptFile($baseUrl . "/js/jquery.smartWizard-2.0.min.js", CClientScript::POS_END);
				Yii::app() ->clientScript -> registerScript('initSmartwizard','
