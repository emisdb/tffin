...
class NestedSetBehavior extends CActiveRecordBehavior
{
	...
	public function __destruct()
	{
     ...
     public function treechild($id,$multi)
 {
  $curnode = $this->getOwner()->findByPk($id);
  if($curnode){
 ...
 if($multi==0)
  {
      $currow=array(
         ...
         . "id='".$children->id."'>"
         .$children->name."</a>",
                'children'=>$this->treechild($children->id,$multi)
     );
     $out[]=$currow;
  }
 else 
 ...
 public function treedata($multi=0)
 {
  $roots = $this->getOwner()->findByPk(0);
  $out = array();
  if($multi==0)
  {
    $currow=array(
         ...
         . "id='".$roots['id']."'>"
         .$roots['name']."</a>",
       'children'=>$this->treechild($roots['id'],$multi));
  }
  else
 {
    $currow=array(
 ...
 public function treeArray()
{
    ...
    $nodes = Yii::app()->db->createCommand()
        ->select('c.id, c.level, c.name as text')
        ->from('pb_set c')
        ->order('c.root, c.lf_key')
        ->queryAll();
    ...
    foreach ($nodes as $node) {
        $item = $node;
        $item['text'] = "<a href='#' onclick=setvalue(this) "
         . "id='".$node['id']."'>".$node['text']."</a>";
      $item['children'] = array();
