...
class TmpDoc extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('ckey, tnum, cfir, ccli, tonum, transport, man', 'length', 'max'=>15),
			array('bsum', 'length', 'max'=>12),
			array('docinfo, cliname, firname, expinfo', 'length', 'max'=>255),
			array('ddat, dodat', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, ckey, ctype, tnum, cfir, state, ddat, ccli, bsum, tonum, dodat, docid, docinfo, cliname, firname, expid, expinfo, transport, man', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tmpDocds' => array(self::HAS_MANY, 'TmpDocd', 'ckey'),
		);
	}
