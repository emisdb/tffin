...
class CountryController extends Controller
{
   ...
   public static function getMenuTree() {
        if (empty(self::$menuTree)) {
            $rows = Country::model()->with('clients')->findAll();
  ...
  //   if (isset($modelRow->getActiveRelation('clients'))) {
       if (isset($modelRow->clients)) {
            $chump = self::getMenuItems($modelRow->clients);
            if ($chump != null)
                $res = array('label' => $modelRow->name, 'items' => $chump, 'url' =>'javascript:void(0);');
            else
                $res = array('label' => $modelRow->name, 'url' => 'javascript:void(0);');
            return $res;
        } 
		else 
		{
			...
			{
                return array('label' => ($modelRow->name), 'url' => 'javascript:void(0);');
            }
        }
    }
